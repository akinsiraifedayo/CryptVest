...
(function () {
  ...
  if (dt_invoice_table.length) {
    var dt_invoice = dt_invoice_table.DataTable({
      ajax: 'app/get-topup-transactions/', // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var $txn_id = full['id'];
            var $phone_number = full['user_profile__phone_number'];
            // Creates full output for row
            var $row_output = '<span>#' + $txn_id + ' ---' + $phone_number +'</span>';
            return $row_output;
          }
        },
        {
          ...
          targets: 2,
          render: function (data, type, full, meta) {
            var $txn_type = full['status'],
              $timestamp = full['created_at'],
              $amount = full['amount'],
              $package__name = full['reference'];

            var roleBadgeObj = {
              debit: `<span class="badge badge-center rounded-pill bg-label-danger w-px-30 h-px-30"><i class="ti ti-info-circle ti-sm"></i></span>`,
              credit: '<span class="badge badge-center rounded-pill bg-label-warning w-px-30 h-px-30"><i class="ti ti-chart-pie ti-sm"></i></span>',
            };
            return (
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, full, meta) {
            var $transaction_type = full['status'];
            return (
              $transaction_type
            );
          }
      ...
      buttons: [
        {
          text: '<i class="ti ti-reload me-md-2"></i><span class="d-md-inline-block d-none">Refresh</span>',
